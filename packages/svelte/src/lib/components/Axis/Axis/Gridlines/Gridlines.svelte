<script lang="ts">
    ...
    import type { IGridlinesBaseProps, IState } from "@chart-io/types";
    ...
    let tickSize = getTickSize(position, plotWidth, plotHeight);

    function renderGridlines() {
        if (gridlinesGroup && scale && scale.domain().length > 0 && scale.range().length > 0) {
