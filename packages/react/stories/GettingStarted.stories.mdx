...
To get started with `@chart-it` first ensure that you've installed the package using one of the following commands:

```
npm install @chart-it/react-d3  //npm
yarn add @chart-it/react-d3     // yarn
pnpm add @chart-it/react-d3     // pnpm
```
...
Here is a basic example of a column chart:
...
<XYChart data={data} margin={margin} width={800} height={500}>
    <YAxis fields={["price"]} />
