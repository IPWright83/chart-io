...
describe("/utils/checks", () => {
  ...
  describe("ensureBandScale", () => {
    it("should return false if not a d3.scaleBand", () => {
      ...
      expect(spy.mock.calls[0][0]).toMatchInlineSnapshot(
        `"@chart-it/react encountered an error. E001: Incompatible scale for a <unit_test />. Are you missing the \`scaleType="band"\` in your <Axis /> or <AutoScale /> component?. You can read more about this http://localhost:6006/?path=/docs/errors-warnings-errors-E001."`
      );
    });
  ...
  describe("ensureNoScaleOverflow for aggregated data", () => {
    it("should return true if the data is larger than the scale", () => {
      ...
      expect(ensureNoScaleOverflow(scale, data, fields, "unit_test")).toBe(
        false
      );

      expect(spy.mock.calls[0][0]).toMatchInlineSnapshot(
        `"@chart-it/react encountered an warning. W001: The scale for unit_test appears too small for the dataset. Are you missing the \`aggregate={true}\` in your <Axis /> or <AutoScale /> component?. You can read more about this http://localhost:6006/?path=/docs/errors-warnings-warnings-W001."`
      );
    });
  ...
  describe("ensureValuesAreUnique", () => {
    ...
    it("should return false if not all the values are unique", () => {
      ...
      expect(spy.mock.calls[0][0]).toMatchInlineSnapshot(
        `"@chart-it/react encountered an warning. W002: There are duplicate values in the x field. This may cause rendering artifacts with a <unit_test>.. You can read more about this http://localhost:6006/?path=/docs/errors-warnings-warnings-W002."`
      );
    });
  });
});
