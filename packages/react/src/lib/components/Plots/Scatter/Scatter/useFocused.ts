import { d3, eventActions } from "@chart-io/core";
import type { IColor, IDispatch, IFocused, IScale } from "@chart-io/core";
 ...
 */
function focus({ dispatch, xScale, yScale, focused }: IScatterFocusProps) {
    if (!focused) return;
 ...
 * @return              A function to set the focused datum
 */
export function useFocused({ xScale, yScale }: Omit<IScatterFocusProps, "focused" | "dispatch">) {
    const { dispatch } = useStore();
    const [focused, setFocused] = useState(null);

    useEffect(() => {
        return focus({ dispatch, xScale, yScale, focused });
