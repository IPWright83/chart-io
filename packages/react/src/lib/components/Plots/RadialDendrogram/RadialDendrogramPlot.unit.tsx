...
describe("RadialDendrogramPlot", () => {
    ...
    describe("using SVG", () => {
        ...
        it("should size each node's circle by its value when nodeRadius is a [min, max] tuple", async () => {
            ...
            const { container } = await renderChart({
                children: (
                    <RadialDendrogramPlot categories={["region", "product"]} value="sales" nodeRadius={[2, 20]} />
                ),
                data: skewedData,
            });
            ...
            const radii = Array.from(container.querySelectorAll("circle.radial-dendrogram-node")).map((node) =>
