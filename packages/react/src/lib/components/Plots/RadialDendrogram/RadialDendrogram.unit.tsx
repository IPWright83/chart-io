    ...
    describe("using SVG", () => {
        it("should render correctly", async () => {
            const { asFragment } = await renderChart({
                children: <RadialDendrogram categories={["region", "product"]} value="sales" />,
                data,
            });
        ...
        it("should hide labels when labels is false", async () => {
            const { container } = await renderChart({
                children: <RadialDendrogram categories={["region", "product"]} value="sales" labels={false} />,
                data,
            });
        ...
        describe("should handle event", () => {
            it("mouseover correctly on a node", async () => {
                ...
                const { container, store } = await renderChart({
                    children: (
                        <RadialDendrogram categories={["region", "product"]} value="sales" onMouseOver={onMouseOver} />
                    ),
                    data,
                });
            ...
            it("click correctly on a leaf node", async () => {
                ...
                const { container, store } = await renderChart({
                    children: <RadialDendrogram categories={["region", "product"]} value="sales" onClick={onClick} />,
                    data,
                });
    ...
    describe("using Canvas", () => {
        it("should render correctly", async () => {
            const { container } = await renderChart({
                children: (
                    <VirtualCanvas>
                        <RadialDendrogram categories={["region", "product"]} value="sales" useCanvas={true} />
                    </VirtualCanvas>
                ),
                data,
            });
