    ...
    describe("using SVG", () => {
        it("should render correctly", async () => {
            const { asFragment } = await renderChart({
                children: <StackedBar y="y" xs={["x", "x2"]} />,
                data,
                scales,
            });
        ...
        describe("should handle event", () => {
            it("mouseover correctly", async () => {
                ...
                const { container, store } = await renderChart({
                    children: <StackedBar y="y" xs={["x", "x2"]} onMouseOver={onMouseOver} />,
                    data,
                    scales,
                });
            ...
            it("mouseexit correctly", async () => {
                ...
                const { container, store } = await renderChart({
                    children: <StackedBar y="y" xs={["x", "x2"]} onMouseOut={onMouseOut} />,
                    data,
                    scales,
                });
            ...
            it("click correctly", async () => {
                ...
                const { container, store } = await renderChart({
                    children: <StackedBar y="y" xs={["x", "x2"]} onClick={onClick} />,
                    data,
                    scales,
                });
    ...
    describe("using Canvas", () => {
        it("should render correctly", async () => {
            const { container } = await renderChart({
                children: (
                    <VirtualCanvas>
                        <StackedBar y="y" xs={["x", "x2"]} useCanvas={true} />
                    </VirtualCanvas>
                ),
                data,
                scales,
