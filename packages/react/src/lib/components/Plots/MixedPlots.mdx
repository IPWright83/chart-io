...
When using a discrete plot such as a Column plot, @chart-io will attempt to manually calculate the width of bars rather than relying on the D3 scale functionality. This relies on the first 2 data points having valid values. If it is unable to do so it will display an error in the console. It will again als attempt to align the data with any other plots.
