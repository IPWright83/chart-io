...
| `zoomable`      | `boolean`  | `false`               | Should a click on a non-leaf slice zoom in and refocus on its subtree? Add `breadcrumb` to the wrapping `<RadialChart>` to also show the current zoom path as a clickable trail. |
...
Set `zoomable` to let a click on a non-leaf slice zoom in and refocus the chart on just its subtree -
the layout re-runs framed to that slice, so its remaining rings expand to fill the available radius.
Clicking the current innermost ring's slice zooms back out one level.

Add `breadcrumb` to the wrapping `<RadialChart>` to also show the current zoom path as a clickable
trail, letting you jump back to any ancestor level (or all the way out via "All") in one click:

<Canvas of={StackedDonutStories.Zoomable} />

```jsx
<RadialChart breadcrumb={true}>
    <StackedDonut categories={["continent", "country", "sector"]} value="gdp" zoomable={true} />
</RadialChart>
```
