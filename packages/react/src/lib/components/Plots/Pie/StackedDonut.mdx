...
represents the `x` category, and the outer ring subdivides each `x` category by `x2`, with each slice's
angle proportional to its value. It should be used within a `<RadialChart>`, which is the polar equivalent
of the `<XYChart>`.
...
| --------------- | ---------- | -------------------- | -------------------------------------------------------------------------------------------------------------- |
| **`x`\***       | `string`   | `null`               | The key of the field that contains the inner ring category.                                                    |
| **`x2`\***      | `string`   | `null`               | The key of the field that contains the outer ring subdivision, within each `x` category.                       |
...
> **Note**: Every combination of `x`/`x2` should be unique in the data. If your data has multiple rows for
> the same `x`/`x2` combination, aggregate it first.
