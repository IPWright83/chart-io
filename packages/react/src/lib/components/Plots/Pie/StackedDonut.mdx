...
<Meta title="Charts/RadialCharts/StackedDonut" />
...
| `zoomable`      | `boolean`  | `true`                | Should a click on a non-leaf slice zoom in and refocus on its subtree, and a click on the center hole zoom back out? Add `breadcrumb` to the wrapping `<RadialChart>` to also show the current zoom path as a clickable trail. |
...
`<StackedDonut>` is zoomable by default - clicking a non-leaf slice zooms in and refocuses the chart on
...
nothing's hovered, and clicking that center hole zooms back out one level. Set `zoomable={false}` to
disable this.
