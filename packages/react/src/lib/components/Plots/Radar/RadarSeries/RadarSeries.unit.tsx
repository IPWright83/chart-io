...
describe("RadarSeries", () => {
    const expectedDatum = { skill: "A", value: 50 };
    ...
    const scales = {
        skill: d3.scalePoint().domain(["A", "B", "C", "D"]).range([0, (2 * Math.PI * 3) / 4]),
        value: d3.scaleLinear().domain([0, 100]).range([0, 100]),
    };

    describe("using SVG", () => {
        it("should render correctly", async () => {
            const { asFragment } = await renderChart({
                children: <RadarSeries category="skill" y="value" />,
                data,
                scales,
            });
        ...
        describe("should handle event", () => {
            it("mouseover correctly on a vertex marker", async () => {
                ...
                const { container, store } = await renderChart({
                    children: <RadarSeries category="skill" y="value" onMouseOver={onMouseOver} />,
                    data,
                    scales,
                });
            ...
            it("mouseexit correctly", async () => {
                ...
                const { container } = await renderChart({
                    children: <RadarSeries category="skill" y="value" onMouseOut={onMouseOut} />,
                    data,
                    scales,
                });
            ...
            it("click correctly", async () => {
                ...
                const { container } = await renderChart({
                    children: <RadarSeries category="skill" y="value" onClick={onClick} />,
                    data,
                    scales,
                });
    ...
    describe("using Canvas", () => {
        it("should render correctly", async () => {
            const { container } = await renderChart({
                children: (
                    <VirtualCanvas>
                        <RadarSeries category="skill" y="value" useCanvas={true} />
                    </VirtualCanvas>
                ),
                data,
                scales,
        ...
        it("should handle a mouseover on a vertex marker", async () => {
            ...
            const { container, store } = await renderChart({
                children: (
                    <VirtualCanvas>
                        <RadarSeries category="skill" y="value" onMouseOver={onMouseOver} useCanvas={true} />
                    </VirtualCanvas>
                ),
                data,
                scales,
