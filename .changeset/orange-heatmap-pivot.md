...
Also adds chart-level pivot state (`pivotable`/`pivot`, mirroring `zoomable`/`zoom`) to `@chart-io/core`, and wires the previously-stubbed "Pivot" `<ContextMenu>` action up to it - it now cycles a pivotable chart through grid/rows/columns and is disabled otherwise.
